...
int main(int argc, char *argv[])
{
    ...
    if(my_id == root_process)
    {
    	...
    	for(turns=1;turns<children;turns++)
	    {
			...
			ierr = MPI_Send(&N,1,MPI_INT,turns,send_data_tag,MPI_COMM_WORLD);

			ierr = MPI_Send(&tree,N,MPI_INT,turns,send_data_tag,MPI_COMM_WORLD);
    	}
